...
export default function App() {
  ...
  const [deletedArr, setDeletedArr] = useState(new Set()); // 상태관리
  const handleDelete = (DeleteIndex) => {
    setDeletedArr(new Set([...deletedArr, DeleteIndex]));
  };
  const handleRestore = (RestoreIndex) => {
    setDeletedArr((prev) => {
      const newSet = new Set(prev); // 불변성, 새로운 set 추가
      newSet.delete(RestoreIndex); // 삭제 배열에서 제거
      return newSet;
    });
  };

  const handleRestoreAll = () => {
    setDeletedArr(new Set());
  };
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-8 sm:pb-12">
        ...
        >
          {/* 이미지 1장 렌더링 */}
          {/* 삭제가 되었을 때는 figure 태그에 classname에 
          `opacity-40 grayscale border-dashed border-muted-foreground/30` 클래스를 추가하세요 */}
          {galleryImages.map((image, index) => {
            const isDeleted = deletedArr.has(index);
            ...
            return (
              ...
              >
                <section className="relative overflow-hidden h-full">
                  ...
                  {/* 이미지가 삭제되지 않았을 때만 삭제 버튼을 렌더링합니다. */}
                  {!isDeleted && (
                    <button
      ...
      <footer className="text-center py-6 sm:py-8 px-4 sm:px-6 border-t border-border bg-muted/30">
        <p className="text-muted-foreground text-sm sm:text-base">
          ...
          <span className="block mt-2 text-xs sm:text-sm">
            {deletedArr.size} images hidden •{" "}
            {galleryImages.length - deletedArr.size} displayed
          </span>
        </p>
      </footer>
    </main>
